...
ok.0:
	...
	%vtpm.30 = getelementptr %Main, %Main* %vtpm.29, i32 0, i32 1
	%vtpm.31 = load i32, i32* %vtpm.30
	%vtpm.32 = bitcast %Main* %vtpm.25 to %IO*
	%vtpm.33 = getelementptr %_Main_vtable, %_Main_vtable* %vtpm.28, i32 0, i32 8
	%tmp.0 = load %IO* (%IO*,i32) *, %IO* (%IO*,i32) ** %vtpm.33
	%vtpm.34 = call %IO*(%IO*, i32 ) %tmp.0( %IO* %vtpm.32, i32 %vtpm.31 )
...
entry:
	...
	%vtpm.41 = call i8* @malloc(i32 %vtpm.40)
...
ok.1:
	...
	store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %vtpm.44
	store %Main* %vtpm.42, %Main** %vtpm.38
	%vtpm.45 = getelementptr %Main, %Main* %vtpm.42, i32 0, i32 1
	store i32 247, i32* %vtpm.45
	ret %Main* %vtpm.42

...
@str.0 = internal constant [10 x i8] c"attr1o.cl\00"
@String.0 = local_unnamed_addr constant %String { %_String_vtable* @_String_vtable_prototype, i8* getelementptr inbounds ([10 x i8], [10 x i8]* @str.0, i32 0, i32 0) }
...
entry:
  %vtpm.41.i = tail call i8* @malloc(i32 16)
  %vtpm.43.i = icmp eq i8* %vtpm.41.i, null
  br i1 %vtpm.43.i, label %abort.i, label %Main_main.exit
...
Main_main.exit:                                   ; preds = %entry
  %vtpm.44.i = bitcast i8* %vtpm.41.i to %_Main_vtable**
  store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %vtpm.44.i, align 8
  %vtpm.45.i = getelementptr i8, i8* %vtpm.41.i, i64 8
  %0 = bitcast i8* %vtpm.45.i to i32*
  store i32 247, i32* %0, align 4
  %vtpm.32.i = bitcast i8* %vtpm.41.i to %IO*
  %vtpm.34.i = tail call %IO* @IO_out_int(%IO* nonnull %vtpm.32.i, i32 247)
  ret i32 0
}
...
ok.0:                                             ; preds = %0
  ...
  %vtpm.30 = getelementptr %Main, %Main* %self, i64 0, i32 1
  %vtpm.31 = load i32, i32* %vtpm.30, align 4
  %vtpm.32 = bitcast %Main* %self to %IO*
  %vtpm.33 = getelementptr %_Main_vtable, %_Main_vtable* %vtpm.28, i64 0, i32 8
  %tmp.0 = load %IO* (%IO*, i32)*, %IO* (%IO*, i32)** %vtpm.33, align 8
  %vtpm.34 = tail call %IO* %tmp.0(%IO* nonnull %vtpm.32, i32 %vtpm.31)
  %vtpm.36 = bitcast %Main* %self to %Object*
  ret %Object* %vtpm.36
...
entry:
  %vtpm.41 = tail call i8* @malloc(i32 16)
  %vtpm.43 = icmp eq i8* %vtpm.41, null
  br i1 %vtpm.43, label %abort, label %ok.1

ok.1:                                             ; preds = %entry
  %vtpm.42 = bitcast i8* %vtpm.41 to %Main*
  %vtpm.44 = bitcast i8* %vtpm.41 to %_Main_vtable**
  store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %vtpm.44, align 8
  %vtpm.45 = getelementptr i8, i8* %vtpm.41, i64 8
  %0 = bitcast i8* %vtpm.45 to i32*
  store i32 247, i32* %0, align 4
  ret %Main* %vtpm.42

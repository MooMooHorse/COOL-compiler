...
private:
  ...
  /// let :== let decl (, decl)* block
  std::unique_ptr<LetExprAST> parseLet() {
    ...
    return std::make_unique<LetExprAST>(std::move(loc), std::move(decls),
                                        std::move(body));
  }
  ...
  std::unique_ptr<ExprASTList> parseBlock() {
    ...
    while (lexer.getCurToken() != '}' && lexer.getCurToken() != tok_eof) {
      ...
      } else if (lexer.getCurToken() == tok_let) {
        // Let statement
        auto let = parseLet();
        if (!let)
          return nullptr;
        exprList->push_back(std::move(let));

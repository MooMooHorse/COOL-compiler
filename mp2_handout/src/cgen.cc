...
void CgenClassTable::code_main()
{
    ...
    **/
    ...
    vp.init_constant(".str", const_value(op_type("[25 x i8]"), "Main.main() returned %d\n", true));
    ...
    operand main_result = vp.call({}, i32_type, "Main_main", true, {});

    // Step 4: Get the address of the string "Main.main() returned %d\n"
    operand str_addr = vp.getelementptr(op_type("[25 x i8]"), operand(op_type("[25 x i8]*"), "@.str"), int_value(0), int_value(0), i8ptr_type);

    // Step 5: Call printf with the string and the return value of Main_main()
    std::vector<operand> printf_args = {str_addr, main_result};
    vp.call({}, i32_type, "printf", true, printf_args);
...
void CgenNode::codeGenMainmain()
{
    ...
    vp.define(op_type(INT32), "Main_main", std::vector<operand>());
    ...
    // Return 0
    vp.ret(const_value(op_type(INT32), std::to_string(0), false));

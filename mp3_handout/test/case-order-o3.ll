...
@String.2 = constant %String { %_String_vtable* @_String_vtable_prototype, i8* getelementptr inbounds ([6 x i8], [6 x i8]* @str.2, i32 0, i32 0) }
...
@String.0 = local_unnamed_addr constant %String { %_String_vtable* @_String_vtable_prototype, i8* getelementptr inbounds ([14 x i8], [14 x i8]* @str.0, i32 0, i32 0) }
...
entry:
  ...
  br i1 %vtpm.60.i, label %abort.i, label %ok.0.i
...
ok.0.i:                                           ; preds = %entry
  ...
  %vtpm.49.i = bitcast i8* %vtpm.58.i to %IO*
  %vtpm.51.i = tail call %IO* @IO_out_string(%IO* nonnull %vtpm.49.i, %String* nonnull @String.2)
  ret i32 0
}
...
define %Object* @Main_main(%Main* %self) {
  %tmp.4 = icmp eq %Main* %self, null
  br i1 %tmp.4, label %abort, label %ok.0

ok.0:                                             ; preds = %0
  %vtpm.28 = bitcast %Main* %self to %_Object_vtable**
  %vtpm.29 = load %_Object_vtable*, %_Object_vtable** %vtpm.28, align 8
  %vtpm.30 = getelementptr %_Object_vtable, %_Object_vtable* %vtpm.29, i64 0, i32 0
  %vtpm.31 = load i32, i32* %vtpm.30, align 4
  %.not = icmp eq i32 %vtpm.31, 5
  br i1 %.not, label %case.exit.0, label %case.0.0

case.0.0:                                         ; preds = %ok.0
  %1 = icmp ugt i32 %vtpm.31, 5
  br i1 %1, label %abort, label %case.exit.0

case.exit.0:                                      ; preds = %case.0.0, %ok.0
  %String.1.sink = phi %String* [ @String.2, %ok.0 ], [ @String.1, %case.0.0 ]
  %vtpm.49 = bitcast %Main* %self to %IO*
  %vtpm.50 = getelementptr %_Object_vtable, %_Object_vtable* %vtpm.29, i64 1
  %2 = bitcast %_Object_vtable* %vtpm.50 to %IO* (%IO*, %String*)**
  %tmp.6 = load %IO* (%IO*, %String*)*, %IO* (%IO*, %String*)** %2, align 8
  %vtpm.51 = tail call %IO* %tmp.6(%IO* nonnull %vtpm.49, %String* nonnull %String.1.sink)
  ret %Object* null

abort:                                            ; preds = %case.0.0, %0
  tail call void @abort()
  unreachable
}

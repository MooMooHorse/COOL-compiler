...
operand get_class_vtable_ptr(operand src, CgenNode *src_cls, CgenEnvironment *env)
{
...
operand get_class_tag(operand src, CgenNode *src_cls, CgenEnvironment *env)
{
    ...
    operand tag_ptr = vp.getelementptr(
        src_cls->get_vtable_type_name(),
        vtable_ptr,
        int_value(0),
        int_value(0),
        op_type(op_type(INT32_PTR))
    );
    
    return vp.load(op_type(INT32), tag_ptr);
}
...
operand dispatch_class::code(CgenEnvironment *env)
{
    ...
    operand vtable_ptr = get_class_vtable_ptr(obj, _class, env); // CRITICAL STEP: dynamic vtable lookup
...
operand typcase_class::code(CgenEnvironment *env)
{
        ...
        code_val.get_type().get_id() != INT1)
    {
        ...
        // std::cerr << code_val.get_name() << " " << code_val.get_typename() << std::endl;
    ...
    std::cerr << tag.get_name() << " " << tag.get_typename() << std::endl;
    vp.branch_uncond(header_label);
    string prev_label = header_label;
    vp.begin_block(header_label);


    operand* br_op_ptr = env->remove_alloca();
    env->branch_operand = *(br_op_ptr);
    operand alloca_op = env->branch_operand;
    delete br_op_ptr;
    ...
    for (int i = ct->get_num_classes() - 1; i >= 0; --i)
    {
        for (auto case_branch : cases)
        {
            ...
            {
                string prefix = string("case.") + std::to_string(i) + ".";
                           ...
                           CgenEnvironment *env)
{
    ...
    string sg_label =
        env->new_label(string("src_gte_br") + "." + std::to_string(my_tag) + ".", false);
    string sl_label =
        env->new_label(string("src_lte_mc") + "." + std::to_string(my_tag) + ".", false);
    string exit_label =
        env->new_label(string("br_exit") + "." + std::to_string(my_tag) + ".", false);
    ...
    operand* alloca_op_ptr = env->remove_alloca();
    operand alloca_op = *(alloca_op_ptr);
    op_type alloca_type = alloca_op.get_type().get_deref_type();
    ...
    env->open_scope();
    env->add_binding(name, alloca_op_ptr);
...
void typcase_class::make_alloca(CgenEnvironment *env)
{
    ...
    operand* alloca_op = new operand(alloca_type, env->new_name());
    vp.alloca_mem(*env->cur_stream, alloca_type, *alloca_op);

    env->add_alloca(alloca_op);
...
void branch_class::make_alloca(CgenEnvironment *env)
{
    ...
    operand* alloca_op = new operand(alloca_type.get_ptr_type(), env->new_name());
    vp.alloca_mem(*env->cur_stream, alloca_type, *alloca_op);
    env->add_alloca(alloca_op);
